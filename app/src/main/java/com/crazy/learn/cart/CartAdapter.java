...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        listBaseAdapter.setOnClickDeleteListenter(new OnClickDeleteListenter() {
            @Override
            public void onItemClick(View view, int onePosition, int position) {


                if (list.get(onePosition).getItems().size() == 1) {
                    list.remove(onePosition);


                }else {
                    list.get(onePosition).getItems().remove(position);
        ...
        listBaseAdapter.setOnClickListenterModel(new OnClickListenterModel() {
            ...
            public void onItemClick(boolean isFlang, View view, int onePosition, int position) {
                list.get(onePosition).getItems().get(position).setIscheck(isFlang);//勾选指定的商品checkbox
                int length = list.get(onePosition).getItems().size();//指定店铺下有几个商品
                if (length == 1) {
